...
<main class="flex-1 w-full min-w-0 overflow-y-auto min-h-screen">
            <div class="p-4 sm:p-6 lg:p-8 max-w-2xl">
                ...
                <div class="bg-white rounded-lg shadow p-6 mb-6 border border-gray-100">
                    <dl class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <dd class="text-lg font-semibold text-gray-900">{{ $product->stock_magasin ?? $product->stock_quantity }}</dd>
                        </div>
                        <div>
                ...
                <form action="{{ route('stock.magasin.update', $product) }}" method="POST" class="bg-white rounded-lg shadow p-6">
                    ...
                    @method('PATCH')
                    <div class="mb-6">
                        <label for="stock_magasin" class="block text-sm font-medium text-gray-700 mb-1">Nouvelle quantité en stock</label>
                        <input type="number" name="stock_magasin" id="stock_magasin" min="0" required
                            value="{{ old('stock_magasin', $product->stock_magasin ?? $product->stock_quantity) }}"
