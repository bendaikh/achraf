...
<main class="flex-1 w-full min-w-0">
    <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-10">
        <div class="px-8 py-4 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Suivi et gestion des paiements fournisseurs</p>
            </div>
    ...
    <div class="p-8">
        ...
        <x-table-filters
            :action="route('purchases.payments.index')"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($invoices as $supplierInvoice)
                            @php
                                ...
                                $remaining = max(0, (float) $supplierInvoice->total - $totalPaid);
                            ...
                            <tr class="hover:bg-gray-50 transition duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-gray-900">{{ number_format($supplierInvoice->total, 2) }} {{ $supplierInvoice->currency }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-green-600">{{ number_format($totalPaid, 2) }} {{ $supplierInvoice->currency }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-red-600">{{ number_format($remaining, 2) }} {{ $supplierInvoice->currency }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($status === 'paid')
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        <a href="{{ route('supplier-invoices.payments.index', $supplierInvoice) }}" class="text-indigo-600 hover:text-indigo-900" title="Gérer les paiements">
                                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"></path>
                                            </svg>
                                        </a>
                                        <a href="{{ route('supplier-invoices.show', $supplierInvoice) }}" class="text-blue-600 hover:text-blue-900" title="Voir la facture">
                                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
