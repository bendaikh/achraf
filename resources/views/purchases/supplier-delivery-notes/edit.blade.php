...
<main class="flex-1 w-full min-w-0">
        <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-10">
            <div class="px-8 py-4 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">Modifier le bon de livraison {{ $supplierDeliveryNote->delivery_number }}</p>
                </div>
        ...
        <div class="p-8">
            ...
            <form action="{{ route('supplier-delivery-notes.update', $supplierDeliveryNote) }}" method="POST" id="supplierDeliveryNoteForm">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <select name="supplier_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                ...
                                @foreach($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}" {{ old('supplier_id', $supplierDeliveryNote->supplier_id) == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="currency" value="{{ old('currency', $supplierDeliveryNote->currency) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Date de livraison *</label>
                            <input type="date" name="delivery_date" value="{{ old('delivery_date', $supplierDeliveryNote->delivery_date->format('Y-m-d')) }}" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Réception prévue</label>
                            <input type="date" name="expected_reception_date" value="{{ old('expected_reception_date', $supplierDeliveryNote->expected_reception_date?->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="reference" value="{{ old('reference', $supplierDeliveryNote->reference) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                @foreach(['brouillon' => 'Brouillon', 'validé' => 'Validé', 'annulé' => 'Annulé', 'envoyé' => 'Envoyé', 'accepté' => 'Accepté', 'refusé' => 'Refusé'] as $value => $label)
                                    <option value="{{ $value }}" {{ old('status', $supplierDeliveryNote->status) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="model" value="{{ old('model', $supplierDeliveryNote->model) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea name="remarks" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('remarks', $supplierDeliveryNote->remarks) }}</textarea>
                        </div>
...
SoftNav.whenReady(function() {
    @if($supplierDeliveryNote->items->count() > 0)
    ...
    document.getElementById('supplierDeliveryNoteForm').addEventListener('submit', function(e) {
