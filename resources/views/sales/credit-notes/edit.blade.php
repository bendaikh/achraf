...
<main class="flex-1 w-full min-w-0">
        <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-10">
            <div class="px-8 py-4 flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Modifier un avoir</h2>
                    <p class="text-sm text-gray-600 mt-1">{{ $creditNote->credit_note_number }}</p>
                </div>
                <a href="{{ route('credit-notes.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition duration-150">
                    Retour à la liste
                </a>
            </div>
        </header>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="min-w-0">
                            ...
                            <x-client-select-with-create :selected-id="$creditNote->client_id" :selected-label="$creditNote->client?->selectLabel()" />
                        </div>

                        <div class="min-w-0">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Avoir Numéro</label>
                            <input type="text" value="{{ $creditNote->credit_note_number }}" disabled class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50">
                        </div>

                        <div class="min-w-0">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Devise</label>
                            <input type="text" name="currency" value="{{ old('currency', $creditNote->currency) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div class="min-w-0">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Facture</label>
                            <select name="invoice_id" id="invoice_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                <option value="">AUCUNE SELECTION</option>
                                @foreach($clientInvoices as $invoice)
                                    <option value="{{ $invoice->id }}" @selected(old('invoice_id', $creditNote->invoice_id) == $invoice->id)>{{ $invoice->invoice_number }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="min-w-0">
                            ...
                            <input type="date" name="credit_note_date" value="{{ old('credit_note_date', $creditNote->credit_note_date->format('Y-m-d')) }}" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div class="min-w-0">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Emplacement du stock</label>
                            <input type="text" name="stock_location" value="{{ old('stock_location', $creditNote->stock_location) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea name="remarks" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('remarks', $creditNote->remarks) }}</textarea>
                        </div>
...
$(document).ready(function() {
    ...
    $('#client_id').on('change', function() {
        filterInvoicesByParty(this.value, 'invoice_id', @json(route('invoices.by-client', ['client' => '__PARTY__'])));
    });
});
