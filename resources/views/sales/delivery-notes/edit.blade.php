...
<main class="flex-1 w-full min-w-0">
        <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-10">
            <div class="px-8 py-4 flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Modifier un bon de livraison</h2>
                    <p class="text-sm text-gray-600 mt-1">Modifier le bon de livraison {{ $deliveryNote->delivery_number }}</p>
                </div>
                <a href="{{ route('delivery-notes.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition duration-150">
                    Retour à la liste
                </a>
            </div>
        </header>
        ...
        <div class="p-8">
            ...
            <form action="{{ route('delivery-notes.update', $deliveryNote) }}" method="POST" id="deliveryNoteForm">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <select name="client_id" id="client_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#fdb819] focus:border-transparent">
                                ...
                                @if($deliveryNote->client)
                                    <option value="{{ $deliveryNote->client_id }}" selected>{{ $deliveryNote->client->selectLabel() }}</option>
                                @endif
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" value="{{ $deliveryNote->delivery_number }}" disabled class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Devise</label>
                            <input type="text" name="currency" value="{{ old('currency', $deliveryNote->currency) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Date de livraison *</label>
                            <input type="date" name="delivery_date" value="{{ old('delivery_date', $deliveryNote->delivery_date->format('Y-m-d')) }}" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Date d'expédition</label>
                            <input type="date" name="shipping_date" value="{{ old('shipping_date', $deliveryNote->shipping_date?->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Emplacement du stock</label>
                            <input type="text" name="stock_location" value="{{ old('stock_location', $deliveryNote->stock_location) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="status" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                <option value="brouillon" {{ old('status', $deliveryNote->status) === 'brouillon' ? 'selected' : '' }}>Brouillon</option>
                                <option value="confirmé" {{ old('status', $deliveryNote->status) === 'confirmé' ? 'selected' : '' }}>Confirmé</option>
                                <option value="livré" {{ old('status', $deliveryNote->status) === 'livré' ? 'selected' : '' }}>Livré</option>
                                <option value="annulé" {{ old('status', $deliveryNote->status) === 'annulé' ? 'selected' : '' }}>Annulé</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Modèle</label>
                            <input type="text" name="model" value="{{ old('model', $deliveryNote->model) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Matricule</label>
                            <input type="text" name="matricule" value="{{ old('matricule', $deliveryNote->matricule) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea name="remarks" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('remarks', $deliveryNote->remarks) }}</textarea>
                        </div>
    ...
    row.innerHTML = `
        <td class="px-4 py-3">
            <select name="items[${itemIndex}][product_id]" onchange="fillCommercialProductDetails(this, ${itemIndex})" class="product-select w-full px-2 py-1 border border-gray-300 rounded text-sm" id="product_select_${itemIndex}">
                ...
                ${products.map(p => `<option value="${p.id}" ${p.id == data.product_id ? 'selected' : ''} data-ref="${p.ref || ''}" data-name="${p.name}" data-price-ht="${p.sale_price_ht || 0}" data-price-ttc="${p.sale_price || 0}">${p.name} ${p.ref ? '(' + p.ref + ')' : ''}</option>`).join('')}
            </select>
        </td>
        <td class="px-4 py-3">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('deliveryNoteForm').addEventListener('submit', function(e) {
        ...
        if (itemRows.length === 0) {
            e.preventDefault();
            alert('Veuillez ajouter au moins un article au bon de livraison.');
            return false;
        }
    });
});
