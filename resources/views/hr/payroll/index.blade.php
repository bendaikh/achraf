    ...
    <div class="bg-white rounded-lg shadow overflow-x-auto">
        <table class="min-w-full text-sm">
            <thead class="bg-gray-50 text-xs uppercase text-gray-500"><tr>
                <th class="px-4 py-3 text-left">Période</th><th class="px-4 py-3 text-left">Statut</th><th class="px-4 py-3 text-left">Bulletins</th><th class="px-4 py-3 text-left"></th>
            </tr></thead>
            <tbody>
                @forelse($runs as $run)
                    <tr class="border-t">
                        <td class="px-4 py-3 font-medium">{{ $run->periodLabel() }}</td>
                        <td class="px-4 py-3">{{ $run->statusLabel() }}</td>
                        <td class="px-4 py-3">{{ $run->slips_count }}</td>
                        <td class="px-4 py-3"><a class="text-[#0a5d8a] font-medium" href="{{ route('hr.payroll.show', $run) }}">Ouvrir</a></td>
                    </tr>
                @empty
